...
Shared TypeScript/Deno utility module for Elite AI Broker Edge Functions. Centralizes auth, RPC
client, structured logging with PII sanitizer, exponential-backoff retry, LLM cost tracking, and
...
```ts
import { callRPC, createContext, logger, withRetry } from "jsr:@elite/shared-utils@^0.1.0";
```
...
Default: 3 attempts, exponential 100→200→400ms with ±20% jitter. Hard cap: 5 attempts (throws
`config_error` above). Retryable by default: `ExternalAPIError`, `RateLimitError`.
`RateLimitError.retryAfter` is honored when present.

## Cost tracking

`trackLLMCost` never throws — failures log a warning so cost-tracking outages don't break
user-facing flows.
...
Semver. `0.1.0` is the initial unstable release; expect breaking changes before `1.0.0`.
