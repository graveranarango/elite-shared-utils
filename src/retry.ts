...
const defaultRetryable = (e: Error): boolean => e instanceof ExternalAPIError || e instanceof RateLimitError;
const jitter = (d: number): number => Math.round(d * (0.8 + Math.random() * 0.4));

export async function withRetry<T>(ctx: TenantContext, fn: () => Promise<T>, options: RetryOptions = {}): Promise<T> {
  const attempts = options.attempts ?? 3;
  if (attempts > MAX_ATTEMPTS_HARD) throw new Error(`config_error: attempts=${attempts} exceeds hard cap ${MAX_ATTEMPTS_HARD}`);
  ...
  for (let i = 0; i < attempts; i++) {
    try { return await fn(); }
      ...
      logger.warn(ctx, "withRetry retrying", { attempt: i + 1, of: attempts, delay_ms: delay, error: err.message });
